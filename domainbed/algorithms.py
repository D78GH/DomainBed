...
ALGORITHMS = [
    ...
    'MLDPMCL_MPU' #JP added - Domain-Conditioned Adaptive Prototype Updating for Domain Generalization
    'FishMLPMCL' #JP added - Meta Learning Multi-Learnable Prototype Memory Contrastive Learning using logsumexp
    'CORAL',
    'MMD',
    'DANN',
    'CDANN',
...
class PMCL(ERM):
    ...
    Using MSE to align current estimate of a class centre (batch_proto) with the historical weighted-average class centre (memory_proto)
    Stores historical batch protoypes.
    Built on top of ERM.
    """
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        # Prototype memory
        # Stores the weighted sum of prototypes
        self.prototype_memory = {}

        # Stores the total number of samples contributing to each prototype
    ...
    def memory_alignment_loss(self, batch_protos):
        """
        Align current batch prototypes with the weighted mean prototype memory.
        """
        ...
        for c, p in batch_protos.items():
            ...
            if c in self.prototype_memory:
                ...
                # Recover weighted mean
                memory_proto = (
                    self.prototype_memory[c]
                    / self.prototype_counts[c]
                )

                memory_proto = F.normalize(memory_proto, dim=0)

                loss += F.mse_loss(p, memory_proto)
    ...
    @torch.no_grad()
    def update_memory(self, batch_protos, batch_counts):
        """
        Update prototype memory using a weighted running mean.

        Memory stores:
            numerator = Σ(n_i * prototype_i)
        ...
        for c, proto in batch_protos.items():
            ...
            if c not in self.prototype_memory:

                self.prototype_memory[c] = proto.detach() * n
                self.prototype_counts[c] = n
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        for c in torch.unique(all_y):
            ...
            batch_protos[int(c.item())] = (
                features_norm[mask].mean(dim=0)
            )

            batch_counts[int(c.item())] = (
                mask.sum().item()
            )
        ...
        # Update weighted prototype memory AFTER optimization
        self.update_memory(
            batch_protos,
            batch_counts
        )
...
class MLPMCL(ERM):
    ...
    def prototype_contrastive_loss(self,features,labels):
        ...
        sim=torch.einsum("bd,ckd->bck",features,prototypes)/self.temperature
    ...
    def memory_alignment_loss(self,batch_protos):
        ...
        for c,batch_proto in batch_protos.items():
            if c>=self.num_classes:
                continue
            memory_proto=F.normalize(self.prototypes[c].mean(0),dim=0)
...
class MLDPMCL_softmax(ERM):
    # multiple prototypes with learned softmax weighting/aggregation with lr 5e-5 and a prototype diversity loss added to encourage multiple distinct representative prototypes per class with 0.001 weighting for small regulariser contribution.
    def __init__(self,input_shape,num_classes,num_domains,hparams):
        super().__init__(input_shape,num_classes,num_domains,hparams)
        self.num_classes=num_classes
        self.num_prototypes=hparams.get("num_prototypes",3)
...
class MLPMCL_softmax2(ERM):
    # multiple prototypes with learned softmax weighting/aggregation with lr 1e-5
    def __init__(self,input_shape,num_classes,num_domains,hparams):
        super().__init__(input_shape,num_classes,num_domains,hparams)
        self.num_classes=num_classes
        self.num_prototypes=hparams.get("num_prototypes",3)
...
class MLADPMCL_softmax(ERM):
    ...
    # identity prototype contrastive loss is weighted by estimated domain alignment;
    # class-aware prototype contrastive loss increases with larger domain shift.
    def __init__(self,input_shape,num_classes,num_domains,hparams):
        super().__init__(input_shape,num_classes,num_domains,hparams)
        self.num_classes=num_classes
        self.num_prototypes=hparams.get("num_prototypes",3)
    ...
    def compute_domain_shift(self,domain_features):
        ...
        for f in domain_features:
            domain_means.append(F.normalize(f.mean(0),dim=0))
        domain_means=torch.stack(domain_means)
        distances=[]
        for i in range(len(domain_means)):
            for j in range(i+1,len(domain_means)):
    ...
    def class_prototype_loss(self,features,labels):
        ...
        for f,y in zip(features,labels):
            logits=torch.matmul(all_prototypes,f)/self.temperature
            positive_mask=torch.zeros(self.num_classes*self.num_prototypes,device=f.device)
            start=y*self.num_prototypes
            end=start+self.num_prototypes
            positive_mask[start:end]=1.0
            log_prob=F.log_softmax(logits,dim=0)
    ...
    def update(self,minibatches,unlabeled=None):
        ...
        for x,y in minibatches:
            ...
            domain_features.append(f)
        domain_shift=self.compute_domain_shift(domain_features)
        alpha=torch.sigmoid(-(domain_shift-self.shift_tau)/self.shift_scale)
...
class MLDPMCL_metaprotoupdate(ERM):
    # multiple prototypes with domain-conditioned learned prototype updates
    def __init__(self,input_shape,num_classes,num_domains,hparams):
        super().__init__(input_shape,num_classes,num_domains,hparams)
        self.num_classes=num_classes
        self.num_prototypes=hparams.get("num_prototypes",3)
        ...
        self.div_weight=hparams.get("div_weight",0.001)
        self.label_smoothing=hparams.get("label_smoothing",0.1)
        self.proto_update_scale=hparams.get("proto_update_scale",0.1)
        ...
        self.optimizer=torch.optim.Adam(
            [
                ...
                {"params":self.classifier.parameters(),"lr":base_lr},
                {"params":[self.prototypes],"lr":base_lr*0.2},
                {"params":self.proto_updater.parameters(),"lr":base_lr},
            ],
            weight_decay=self.hparams["weight_decay"],
        )
    ...
    def compute_domain_statistics(self,features):
        mean_shift=torch.norm(F.normalize(features.mean(0),dim=0))
        variance=features.var(dim=0).mean()
        return mean_shift,variance

    def adapt_prototypes(self,batch_protos,domain_shift,variance):
        prototypes=F.normalize(self.prototypes,dim=2)
        adapted=[]
        for c in range(self.num_classes):
            class_adapted=[]
            for k in range(self.num_prototypes):
                proto=prototypes[c,k]
                if c in batch_protos:
                    batch_proto=batch_protos[c]
                else:
                    batch_proto=proto
                shift=torch.tensor([domain_shift],device=proto.device)
                var=torch.tensor([variance],device=proto.device)
                updater_input=torch.cat([proto,batch_proto,shift,var],dim=0)
                delta=self.proto_updater(updater_input)
                class_adapted.append(proto+self.proto_update_scale*delta)
            adapted.append(torch.stack(class_adapted))
        return F.normalize(torch.stack(adapted),dim=2)

    def prototype_contrastive_loss(self,features,labels,prototypes):
        features=F.normalize(features,dim=1)
        weighted_protos=[]
        for f,y in zip(features,labels):
            class_protos=prototypes[y]
    ...
    def memory_alignment_loss(self,batch_protos,prototypes):
        if len(batch_protos)==0:
    ...
    def update(self,minibatches,unlabeled=None):
        ...
        for c in torch.unique(all_y):
            mask=all_y==c
            batch_protos[int(c.item())]=F.normalize(features_norm[mask].mean(0),dim=0)
        domain_shift,variance=self.compute_domain_statistics(features.detach())
        adapted_prototypes=self.adapt_prototypes(batch_protos,domain_shift,variance)
        proto_loss=self.prototype_contrastive_loss(features,all_y,adapted_prototypes)
        mem_loss=self.memory_alignment_loss(batch_protos,adapted_prototypes)
        div_loss=self.prototype_diversity_loss()
        loss=ce+self.proto_weight*proto_loss+self.mem_weight*mem_loss+self.div_weight*div_loss
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        return {"loss":loss.item(),"ce_loss":ce.item(),"proto_loss":proto_loss.item(),"mem_loss":mem_loss.item(),"div_loss":div_loss.item(),"domain_shift":domain_shift.item(),"variance":variance.item()}

    def predict(self,x):
        return self.network(x)


class MLDPMCL_MPU(ERM):
    # multiple prototypes with domain-conditioned learned prototype updates
    def __init__(self,input_shape,num_classes,num_domains,hparams):
        super().__init__(input_shape,num_classes,num_domains,hparams)
        self.num_classes=num_classes
...
class FishMLPMCL(Algorithm):
    ...
    def memory_alignment_loss(self,batch_protos,prototypes):
        ...
        for c,batch_proto in batch_protos.items():
            if c>=self.num_classes:
                continue
            memory_proto=F.normalize(prototypes[c].mean(0),dim=0)
            loss+=F.mse_loss(batch_proto,memory_proto)
            count+=1
        if count==0:
            return torch.tensor(0.0,device=prototypes.device)
    ...
    def update(self,minibatches,unlabeled=None):
        self.create_clone(minibatches[0][0].device)
        for x,y in minibatches:
            logits,features=self.network_inner(x)
            ce=self.ce_loss(logits,y)
            proto=self.prototype_contrastive_loss(features,y,self.network_inner.prototypes)
            features_norm=F.normalize(features,dim=1)
            batch_protos={}
            for c in torch.unique(y):
                mask=y==c
                batch_protos[int(c.item())]=F.normalize(features_norm[mask].mean(0),dim=0)
            mem=self.memory_alignment_loss(batch_protos,self.network_inner.prototypes)
            loss=ce+self.proto_weight*proto+self.mem_weight*mem
            self.optimizer_inner.zero_grad()
            loss.backward()
            self.optimizer_inner.step()
        self.optimizer_inner_state=self.optimizer_inner.state_dict()
        ...
        self.network.reset_weights(meta_weights)
        return {"loss":loss.item(),"ce_loss":ce.item(),"proto_loss":proto.item(),"mem_loss":mem.item()}
